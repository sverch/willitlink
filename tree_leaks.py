...
def find_direct_leaks(g, archive_name):
    ...
    for full_archive_name in full_archive_names:
        ...
        symbols_needed = find_symbol_dependencies_with_file(g, full_archive_name)
        ...
        leaks = set()
        for (symbol_needed, file_needing) in symbols_needed:
            if symbol_needed not in symbols_found:
                leaks.add((symbol_needed, file_needing))

        o = []
        for (symbol_leak, file_leaking) in leaks:
            try:
                if (len(g.get('symbol_to_file_sources', symbol_leak)) > 0):
                    result = {}
                    result['symbol'] = symbol_leak
                    result['file'] = file_leaking
                    o.append(result)
            except KeyError:
                pass
